...
@Form.create()
class ConfigGroupForm extends PureComponent {
      ...
      >
        {
          ...
          <FormItem labelCol={{span: 5}} wrapperCol={{span: 15}} label="组名称">
            {form.getFieldDecorator('remark', {
              initialValue: formValues.remark || '',
              rules: [{required: true, message: '组名称不能为空！'}],
            })(<TextArea rows={1} placeholder="组名称" />)}
          </FormItem>
        }
        {
          !hiddenFields.includes('name') &&
            <FormItem labelCol={{span: 5}} wrapperCol={{span: 15}} label="组标识">
              {form.getFieldDecorator('name', {
                initialValue: formValues.name || '',
                rules: [{required: true, message: '组标识不能为空！'}],
              })(<Input placeholder="用于取组信息, 如site" />)}
            </FormItem>
        }
